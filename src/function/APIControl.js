...
// The value from Geolocation is passed to APIs of VisualCrossing and Nominatim to fetch weather forecast
export async function retrieveBtnData(position) {
  try {
    ...
    const allResponse = await customPromiseAllWait([
      fetch(
        `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${latitude},${longitude}/next5days?unitGroup=metric&key=${myKey}`,
        { mode: "cors" },
      ),
      fetch(
    ...
    if (allResponse[0].status === 400) {
      showBtnLocationError();
      throw new Error(
        `Error parameter for the location: ${latitude}, ${longitude}`,
      );
    }
    ...
    // Use first element in array of locationData object to get location
    const {
      features: [
        {
          properties: {
            address: { country, city, suburb },
          },
        },
      ],
    } = locationData;
    ...
    const selectedWeatherData = {
      locationAddress: `${suburb}, ${city}, ${country}`,
      recordDate,
      recordTime,
      temp,
      conditions,
    ...
    console.log("Error happens in retrieveBtnData");
    // If there is some error thrown out when requesting the weather data, re-throw the error message
    throw error;
  }
}
